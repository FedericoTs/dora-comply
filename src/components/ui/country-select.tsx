 ...
 *
 * Searchable dropdown for ISO 3166-1 alpha-2 country codes
 * Used for headquarters_country, jurisdiction, and other country fields
 */
...
// EU member states for quick filtering
...
export function isEUCountry(code: string): boolean {
  return EU_COUNTRIES.includes(code.toUpperCase());
}
...
export function CountrySelect({
  ...
  const filteredCountries = React.useMemo(() => {
    if (euOnly) {
      return COUNTRIES.filter(c => EU_COUNTRIES.includes(c.code));
    }
    if (eeaOnly) {
      return COUNTRIES.filter(c => EEA_COUNTRIES.includes(c.code));
    }
    return COUNTRIES;
  }, [euOnly, eeaOnly]);
...
interface CountrySelectFieldProps extends CountrySelectProps {
